...
export class AgentOrchestrator {
  ...
  private async executeToolCall(
    ...
    try {
      ...
      if (externalPathArg && call.name !== "import_external_file") {
        const approved = await this.approvalManager.confirmExternalPathAccess(
          call.name,
          externalPathArg,
          steps,
        );
      ...
      if (call.name === "run_command") {
        ...
        if (command) {
          const approved = await this.approvalManager.confirmCommand(
            command,
            steps,
            "tool",
          );
        ...
        if (sourcePath) {
          ...
          const destinationPath =
            typeof args.destinationPath === "string"
              ...
              : undefined;
          const approved = await this.approvalManager.confirmExternalFileImport(
            sourcePath,
            destinationPath,
            mode,
            steps,
  ...
  private async runAutoValidation(
    ...
    for (const [index, command] of commands.entries()) {
      ...
      if (parsed?.exitCode && parsed.exitCode !== 0) {
        const diagnostics = await getDiagnosticsForValidationCommand(command);
        return { failedPrompt: buildFailurePrompt(parsed, diagnostics) };
      }
    }
  ...
  private async persistSession(): Promise<void> {
    try {
      this.sessionId = await saveSession({
        id: this.sessionId,
    ...
    for (let i = 0; i < maxIterations; i++) {
      ...
      if (response.toolCalls.length === 0) {
        if (hasModifiedFiles && !hasValidated) {
          const validationResult = await this.runAutoValidation(
            i,
            steps,
      ...
      if (canParallelize) {
        const results = await Promise.allSettled(
          response.toolCalls.map((call) =>
            this.executeToolCall(call, steps, diffs, modifiedPaths, {
              hasModifiedFiles,
              hasValidated,
              autoFixRounds,
            }),

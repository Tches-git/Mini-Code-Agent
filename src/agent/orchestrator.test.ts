...
const fakeTools = vi.hoisted(() => {
  const tools = [
    ...
    },
    {
      ...
      async execute() {
        return { message: "已写入", diff: { path: "test.ts", summary: "写入", diff: "+new content" } };
      }
    },
    {
      name: "run_command",
      ...
      async execute(input: Record<string, unknown>) {
        return JSON.stringify({ command: input.command, exitCode: 0, stdout: "ok", stderr: "" });
      }
    },
    {
      name: "failing_tool",
...
vi.mock("../tools/index.js", () => ({
  tools: fakeTools,
  getToolMap: () => new Map(fakeTools.map((t: { name: string }) => [t.name, t]))
}));
...
describe("AgentOrchestrator", () => {
  ...
  it("无 tool_calls 时直接返回文本", async () => {
    mockChatStream.mockImplementationOnce(
      async (_msgs: unknown, _tools: unknown, onEvent: (e: { type: string; text: string }) => void) => {
    ...
    mockChatStream.mockImplementation(
      async (_msgs: unknown, _tools: unknown, onEvent: (e: { type: string; text?: string }) => void) => {
        ...
        if (callCount === 1) {
          ...
          return {
            text: "",
            toolCalls: [{ id: "call-1", name: "read_file", argumentsText: '{"path":"src/index.ts"}' }]
          };
        }
  ...
  it("工具执行失败后错误回传到消息中", async () => {
    let callCount = 0;
    mockChatStream.mockImplementation(
      async (_msgs: unknown, _tools: unknown, _onEvent: (e: { type: string }) => void) => {
        callCount++;
        if (callCount === 1) {
          return {
            text: "",
            toolCalls: [{ id: "call-1", name: "failing_tool", argumentsText: "{}" }]
          };
        }
    ...
    const events: string[] = [];
    const agent = new AgentOrchestrator({
      onEvent: (event) => { if (event.type === "tool_error") events.push(event.type); }
    });
      ...
      async (_msgs: unknown, _tools: unknown, _onEvent: (e: { type: string }) => void) => {
        ...
        if (callCount === 1) {
          return {
            text: "",
            toolCalls: [
              { id: "call-1", name: "read_file", argumentsText: '{"path":"a.ts"}' },
              { id: "call-2", name: "read_file", argumentsText: '{"path":"b.ts"}' }
            ]
          };
        }
  ...
  it("达到最大执行轮数时返回预算提示", async () => {
    mockChatStream.mockImplementation(
      async (_msgs: unknown, _tools: unknown, _onEvent: (e: { type: string }) => void) => {
        return {
          text: "",
          toolCalls: [{ id: `call-${Math.random()}`, name: "read_file", argumentsText: '{"path":"x.ts"}' }]
        };
      }
    );

import type { DiffEntry, ToolDefinition, ToolResult } from "../types/agent.js";
...
export async function executeToolCall(
  ...
  try {
    ...
    if (externalPathArg && call.name !== "import_external_file") {
      const approved =
        await dependencies.approvalManager.confirmExternalPathAccess(
          call.name,
          externalPathArg,
          steps,
        );
      ...
      if (sourcePath) {
        ...
        const destinationPath =
          typeof args.destinationPath === "string"
            ...
            : undefined;
        const approved =
          await dependencies.approvalManager.confirmExternalFileImport(
            sourcePath,
            destinationPath,
            mode,
            steps,

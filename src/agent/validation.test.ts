...
import { buildFailurePrompt, getValidationPlan, isValidationCommand, parseCommandResult } from "./validation.js";
...
describe("validation helpers", () => {
  it("parseCommandResult 能解析 JSON 字符串", () => {
    expect(parseCommandResult('{"command":"npm run build","exitCode":1}')).toEqual({ command: "npm run build", exitCode: 1 });
  });
  ...
  it("buildFailurePrompt 会包含命令、退出码和输出", () => {
    const prompt = buildFailurePrompt({ command: "npm run build", exitCode: 2, stdout: "out", stderr: "err" });
...
describe("getValidationPlan", () => {
  ...
  it("修改源码时优先选择 lint/build", async () => {
    vi.spyOn(fs, "readFile").mockResolvedValue(JSON.stringify({ scripts: { lint: "eslint .", build: "tsc", test: "vitest run" } }));
  ...
  it("修改配置时执行完整验证", async () => {
    ...
    expect(plan.commands).toEqual(["npm run lint", "npm run test", "npm run build"]);
  });

  it("占位 test 脚本会被忽略", async () => {
    vi.spyOn(fs, "readFile").mockResolvedValue(JSON.stringify({ scripts: { lint: "eslint .", build: "tsc", test: "echo no test specified && exit 1" } }));

...
- **工具调用闭环**：不只是生成文本，而是让 LLM 自主调用文件操作、搜索、project map、命令执行、Git、diagnostics 等工具，形成完整的任务执行链路；其中 project map 可快速返回关键文件、顶层符号、relations/dependsOn、importedBy、符号级 references、externalDeps，以及 entry/core/leaf 等结构信息
...
2. **自动执行验证**：依次运行 `lint` → `test` → `build`（仅运行项目中实际存在的脚本）；如果直接改了测试文件，会优先只跑受影响测试
3. **验证回退与失败重放**：如果定向测试命令不被底层 runner 支持，会自动回退到完整 `test`；如果完整测试失败，还会从输出里提取失败测试文件并优先重放
4. **结构化读取错误**：对于 `lint` / `build` 类失败，额外读取结构化 diagnostics，提取文件、行号、错误级别、错误码与消息
5. **失败时自动修复**：将错误摘要与 diagnostics 一并回灌给 LLM，要求它定位问题并修复，最多尝试 2 轮
...
| `project_map` | 生成项目结构地图，返回关键文件、顶层符号、relations/dependsOn、importedBy、references、externalDeps 与 entry/core/leaf/module 角色 | 工作区外需确认 |
...
- Agent 先搜索和读取相关文件
- 修改后自动触发验证；如果改到测试文件，会优先只跑受影响测试
- 定向测试命令不受支持时会自动回退到完整 `test`
- 验证失败时，不只看到原始 stderr，还会补充结构化 diagnostics
- 完整测试失败后，会从输出中提取失败测试文件并优先重放
...
- 会话持久化与恢复
- 自动验证计划生成（含受影响测试优先执行）
- 验证回退、失败测试重放与 diagnostics 解析/失败回灌
